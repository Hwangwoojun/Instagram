...
const checkEmailOrPhone = () => {
  ...
  if (emailOrPhone.value.includes('@')) {
    axios
      .get(`http://localhost:8080/check-email/${emailOrPhone.value}`)
      .then((res) => {
        if (res.data) {
    ...
    axios
      .get(`http://localhost:8080/check-phone/${phoneWithoutDash}`)
      .then((res) => {
        if (res.data) {
...
const validateUserName = () => {
  ...
  axios
    .get(`http://localhost:8080/check-username/${userName.value}`)
    .then((res) => {
      if (res.data) {
