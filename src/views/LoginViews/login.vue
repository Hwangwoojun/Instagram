...
const checkLogin = () => {
  errorMessage.value = '' // 로그인 시 오류 메시지 초기화
  if (emailOrPhone.value.trim() === '' || password.value.trim() === '') {
    alert('전화번호 또는 이메일, 비밀번호를 입력하세요.') // alert 창으로 경고 표시
  } else {
    const loginData = {
    ...
    // 로그인 요청을 보내는 부분
    axios
      .post('http://localhost:8080/login', loginData) // Spring Boot 백엔드 로그인 URL에 POST 요청
      .then((res) => {
        ...
        if (res.data && res.data.status !== 'error') {
          ...
          Cookies.set('id', res.data.id) // 쿠키에 사용자 이름 저장
          console.log(Cookies.set('userName', res.data.user_name))
          console.log(Cookies.set('id', res.data.id))
          userName.value = res.data.user_name // 성공 시 사용자 이름을 Vuex 스토어에 저장
          isModalOpen.value = true // 모달 열기
          localStorage.setItem('token', res.data.token) // localStorage에 토큰 저장
