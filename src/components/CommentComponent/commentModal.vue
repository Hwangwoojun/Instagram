...
const props = defineProps({
  ...
  imageUrls: Array, // 게시물 이미지 URL 배열
  userName: String, // 사용자 이름 prop 추가
  content: String, // 게시물 내용 prop 추가
  postDate: String // post_date prop 추가
})
...
const fetchComments = () => {
  ...
  axios
    .get(`http://localhost:8080/comment/${props.postId}`) // postId로 댓글 가져오기
    .then((res) => {
      if (res.data.message) {
...
const fetchLikeInfo = () => {
  ...
  // 좋아요 여부 확인
  axios
    .post('http://localhost:8080/like/check', params)
    .then((res) => {
...
const fetchLikeCount = () => {
  axios
    .get(`http://localhost:8080/count/${props.postId}`)
    .then((res) => {
  ...
  axios
    ...
    .then((res) => {
      ...
      if (currentlyLiked) {
        // 좋아요 삭제 요청
        axios
          .delete('http://localhost:8080/like', { data: params })
          .then(() => {
            isLiked.value = false
            likeCount.value -= 1
          })
...
.post_image {
  width: 920px; /* 부모 요소의 너비에 맞춤 */
  height: 920px; /* 부모 요소의 높이에 맞춤 */ /* 이미지 비율 유지하면서 부모 요소에 맞춤 */
  object-fit: contain;
...
.heart_gif {
  ...
  top: 40%; /* 중앙 정렬 */
  left: 50%;
  transform: translate(-100%, -20%); /* 정확한 중앙에 배치 */
...
.image_slider {
  width: 920px;
  position: relative;
  overflow: hidden; /* 이미지 슬라이더가 넘치지 않도록 설정 */
}

.slider {
  display: flex;
  transition: transform 0.5s ease; /* 슬라이더 전환 애니메이션 */
}

.slide {
  min-width: 100%; /* 각 이미지가 슬라이더의 전체 너비를 차지하도록 설정 */
}

.post_image {
  width: 100%; /* 이미지가 슬라이더의 전체 너비를 차지하도록 설정 */
}
.prev {
  width: 30px;
  position: absolute;
