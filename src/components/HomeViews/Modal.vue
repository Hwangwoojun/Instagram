...
const File_Change = (event) => {
  ...
  if (files.length > 0) {
    ...
    selectedFiles.value = [...selectedFiles.value, ...files] // 변경된 부분
...
const removeImage = (index) => {
  imageSrc.value.splice(index, 1) // 이미지 배열에서 삭제
  if (currentIndex.value >= index) {
    currentIndex.value = Math.max(0, currentIndex.value - 1) // 인덱스 조정
  }
...
// 뒤로 가기 이미지 클릭 시 이미지 삭제
const arrow_img_delete_Modal = () => {
  if (imageSrc.value.length > 0) {
    arrow_Delete_Modal_img.value = true // 이미지가 선택된 경우 이미지 삭제 모달 열기
  }
}
...
const dragStart = (index) => {
  ...
  isDragging.value = true // 드래그 중 상태로 설정
}

// 드롭 시 실행되는 함수 (이미지를 위치만 변경)
const drop = (index) => {
  if (selectedImageIndex.value === null) return // 드래그한 이미지가 없는 경우 리턴

  // 드래그된 이미지의 위치를 새로운 위치로 변경
  const draggedImage = imageSrc.value[selectedImageIndex.value]
  // 드래그된 이미지 삭제
  imageSrc.value.splice(selectedImageIndex.value, 1)
  // 새로운 위치에 추가
  ...
  // 기존 파일을 그대로 유지하고 다시 업로드되지 않도록 selectedFiles 갱신을 막음
  // selectedFiles.value는 변경하지 않음으로써 다시 업로드를 방지
}
