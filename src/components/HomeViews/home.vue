<template>
  <div class="home">
    <div v-for="post in posts" :key="post.post_id" class="post">
      ...
      <img src="../../../public/assets/ellipsis-solid.svg" alt="이미지" class="ellipsis" />
...
const fetchPosts = () => {
  ...
  axios
    .post('http://localhost:8080/posts', { page: page.value })
    .then((res) => {
...
const fetchAllCommentCounts = () => {
  posts.value.forEach((post) => {
    axios
      .get(`http://localhost:8080/post/count/${post.post_id}`)
      .then((res) => {
...
// 댓글 모달 열기
const openCommentModal = (postId, imageUrls, userName, content, postDate) => {
...
const initializeSlider = (post) => {
  post.currentImageIndex = 0 // 현재 이미지 인덱스 초기화
}
...
.heart_gif {
  ...
  top: 40%; /* 중앙 정렬 */
  left: 50%;
  transform: translate(-50%, -50%); /* 정확한 중앙에 배치 */
